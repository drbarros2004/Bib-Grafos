...
int main() {
    ...
    string saida_arquivo = "estudo_de_caso2_vector.txt";
    string saida_arquivo_2 = "estudo_de_caso2_matriz.txt";

    vector<vector<int>> grafo2_vector = txt_to_adjacency_vector(nome_arquivo, saida_arquivo);
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_dfs_vector += DFS_tree_vector_with_execution_time(grafo2_vector, rand() % grafo2_vector.size());
    }
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_bfs_vector += BFS_tree_vector_with_execution_time(grafo2_vector, rand() % grafo2_vector.size());
    }
    ...
    BFS_tree_vector(grafo2_vector, 1, "arvoreBFS_1.txt");
    BFS_tree_vector(grafo2_vector, 2, "arvoreBFS_2.txt");
    BFS_tree_vector(grafo2_vector, 3, "arvoreBFS_3.txt");
    DFS_tree_vector(grafo2_vector, 1, "arvoreDFS_1.txt");
    DFS_tree_vector(grafo2_vector, 2, "arvoreDFS_2.txt");
    DFS_tree_vector(grafo2_vector, 3, "arvoreDFS_3.txt");

    // Distâncias e diâmetros:

    Distancia_vector(grafo2_vector, 10, 20);
    Distancia_vector(grafo2_vector, 10, 30);
    Distancia_vector(grafo2_vector, 20, 30);
    Diametro_vector(grafo2_vector);

    // Desalocar o grafo2_vector
    grafo2_vector = vector<vector<int>>();
    arquivo_de_saida.close();

    // Converte a matriz para o próximo teste
    vector<vector<bool>> grafo2_matrix = txt_to_adjacency_matrix(nome_arquivo, saida_arquivo_2);
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_bfs_matrix += BFS_tree_matrix_with_execution_time(grafo2_matrix, rand() % grafo2_matrix.size());
    }
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_dfs_matrix += DFS_tree_matrix_with_execution_time(grafo2_matrix, rand() % grafo2_matrix.size());
    }
