...
int main() {
    ...
    string nome_arquivo = "grafo_4.txt";
    string saida_arquivo = "estudo_de_caso4_vector.txt";
    string saida_arquivo_2 = "estudo_de_caso4_matriz.txt";

    vector<vector<int>> grafo4_vector = txt_to_adjacency_vector(nome_arquivo, saida_arquivo);
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_dfs_vector += DFS_tree_vector_with_execution_time(grafo4_vector, rand() % grafo4_vector.size());
    }
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_bfs_vector += BFS_tree_vector_with_execution_time(grafo4_vector, rand() % grafo4_vector.size());
    }
    ...
    BFS_tree_vector(grafo4_vector, 1, "arvoreBFS_1.txt");
    BFS_tree_vector(grafo4_vector, 2, "arvoreBFS_2.txt");
    BFS_tree_vector(grafo4_vector, 3, "arvoreBFS_3.txt");
    DFS_tree_vector(grafo4_vector, 1, "arvoreDFS_1.txt");
    DFS_tree_vector(grafo4_vector, 2, "arvoreDFS_2.txt");
    DFS_tree_vector(grafo4_vector, 3, "arvoreDFS_3.txt");

    // Distâncias e diâmetros:

    Distancia_vector(grafo4_vector, 10, 20);
    Distancia_vector(grafo4_vector, 10, 30);
    Distancia_vector(grafo4_vector, 20, 30);
    Diametro_vector(grafo4_vector);

    // Desalocar o grafo4_vector
    grafo4_vector = vector<vector<int>>();
    arquivo_de_saida.close();

    // Converte a matriz para o próximo teste
    vector<vector<bool>> grafo4_matrix = txt_to_adjacency_matrix(nome_arquivo, saida_arquivo_2);
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_bfs_matrix += BFS_tree_matrix_with_execution_time(grafo4_matrix, rand() % grafo4_matrix.size());
    }
    ...
    for (int i = 0; i < 100; i++) {
        tempo_total_dfs_matrix += DFS_tree_matrix_with_execution_time(grafo4_matrix, rand() % grafo4_matrix.size());
    }
